...
`observations.md` (#16–#18); this is the forward-looking design thinking, especially the *how would the
...
- **Looter camps feel done.** Functional, they look decent, properly clearable, and trap-designable.
  As a *content + flee-or-fight* system it lands. (→ `observations.md` #16.)
- **The build is not balanced — at all.** Parts are simply *available*, and a few have enough scrap to
  ...
  tiers). Logging it as the looming system, with the current-state gap in `observations.md` #17.
...
**Status:** RAW idea at capture. *(Addendum at landing: the v1 fading drive-mark has since **shipped** —
[`specs/track-marks-spec.md`](specs/track-marks-spec.md), `features/tracks/` — exactly as the
deliberately-minimum cut sketched above, trail-emitter seam included. The life-trails future half stays
captured in `world-progression-guidance.md` §3a. This session is the original capture, kept for the dated
history; it reached `main` late because its branch was parked.)*
