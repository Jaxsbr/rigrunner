...
import { PART_COSTS } from '../content/part-costs';
import { PART_SHOP_STOCK } from '../content/part-shop';
import { PARTS_CATALOG, partDef, spawnEnginePart } from '../content/parts-catalog';
...
describe('sellPart', () => {
  ...
  it('sells an inventory-held loose part, credits scrap, and destroys the part entity', () => {
    const world = setup(0);
    const part = spawnEnginePart(world, partDef('container-shell')!);
  ...
  it('refuses to sell an entity that is not in inventory', () => {
    const world = setup(0);
    const part = spawnEnginePart(world, partDef('container-rim')!);
