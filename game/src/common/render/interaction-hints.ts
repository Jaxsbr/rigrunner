 ...
 * owns only the billboard sprites and their eased opacity.
 *
 * Like `ZoneOverlays`, this is generic render INFRASTRUCTURE (ADR-003 §4): it knows nothing about
 * which feature a bubble belongs to. Each feature contributes its bubbles as a plain `HintSpec[]`
 * (carrying the label and the height to ride at — see `features/<x>/overlays`) and `main.ts`
 * concatenates them and calls `sync`, so the shared render tier never imports a feature. Bubbles are
 * created lazily on first sight and dropped when their owner stops appearing (e.g. a pile emptied).
 *
 * Each bubble is a camera-facing THREE.Sprite drawn from a canvas. depthTest is off so the bubble
 * reads clearly even against the tall scrap heap.
 */
export interface HintSpec {
  id: EntityId;
  x: number;
  y: number; // world height (metres) above the object's origin to ride the bubble at
  z: number;
  label: string; // e.g. "Press E" / "Hold E"
  active: boolean;
}
...
export class InteractionHints {
  ...
  private readonly textures = new Map<string, THREE.Texture>(); // cached by label — only a few exist

  constructor(private readonly scene: THREE.Scene) {}

  /** Fade every hint toward its entry's gate state and ride it at the given height. `dt` drives the fade. */
  sync(entries: readonly HintSpec[], dt: number): void {
    const seen = new Set<EntityId>();
    for (const e of entries) {
      seen.add(e.id);
      this.upsert(e, dt);
    }
    // Drop bubbles for any owner that no longer appears (destroyed, or its gate component removed).
  ...
  private upsert(e: HintSpec, dt: number): void {
    let hint = this.hints.get(e.id);
    if (!hint) {
      const sprite = new THREE.Sprite(
        new THREE.SpriteMaterial({ map: this.texture(e.label), transparent: true, opacity: 0, depthTest: false, depthWrite: false }),
      );
      ...
      hint = { sprite, opacity: 0, active: e.active };
      this.hints.set(e.id, hint);
    }

    hint.sprite.position.set(e.x, e.y, e.z);
