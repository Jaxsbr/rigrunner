...
export function spawnContainer(world: World, x = 0, z = 0): EntityId {
  ...
  // A directly-spawned container is a base (rusty) one — drawn as its composed Shell + Rim through the
  // shared assembler, both at the rusty grade, so it reads identically to a bench-built rusty container
  // (whose capacity is the same tier-1 CONTAINER_CAPACITY).
  world.add(e, Renderable, {
    shape: 'assembly',
    groupId: 'storage',
    tiers: { 'container-shell': DEFAULT_TIER, 'container-rim': DEFAULT_TIER },
  });
  return e;
}
