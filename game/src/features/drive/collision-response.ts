 ...
 *
 * The model is de-penetration ONLY — it corrects POSITION and never touches the rig's speed. Preserving
 * speed is the load-bearing choice: steering is yaw ∝ speed (the rig arcs like a vehicle, it can't pivot on
 * the spot), so a rig parked at zero speed against a wall could not rotate off it — it would have to fully
 * reverse. By keeping the speed, a held turn pivots the rig off the wall and drives it away, no reverse
 * required, and walls stay forgiving. The pieces:
 ...
 *    cancels that frame's forward over-step, so the mover sits flush against the surface: a clamp to the
 *    surface, not an impulse that overshoots, so there is nothing to oscillate and (no velocity reflection)
 *    nothing bounces.
 *  - Slide falls out for free: only the into-the-surface part of the displacement is undone, the tangential
 *    part survives, so a rig hitting at an angle scrapes along the surface and deflects off rather than sticking.
 *  - Coming to rest is the normal coasting friction's job, not the response's: release the throttle against a
 *    wall and friction (movement.ts) bleeds the rig to a stop at the surface; hold it and the wheels keep
 *    driving against the wall until you steer away.
...
export function collisionResponseSystem(world: World): void {
  ...
  for (const mover of world.query(Transform, Drivetrain, Velocity, Collider)) {
    ...
    for (const s of solids) {
      ...
      // De-penetrate to exactly touching. Position only — speed is left alone so the rig keeps the
      // momentum the turning model needs to steer off the wall (see the header).
