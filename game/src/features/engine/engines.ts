import { PARTS_CATALOG, type PartDef, type EnergyType } from '@common/parts/parts-catalog';
import { ELECTRIC_ENGINE_RECIPE, STEAM_ENGINE_RECIPE } from '@common/parts/recipes';

/**
 * Engines are no longer hand-authored blueprints (the old Mk1/Mk2 placeholders are retired): every
 * engine is COMPOSED from the four catalog sub-parts of one energy type, exactly as the workshop
 * bench assembles them. Its `EngineSpec` + `Weight` are the sum of its parts, so there is a single
 * way an engine comes to exist — `@common/sim/assembly` builds it from these parts.
 *
 * This module owns only the engine's RECIPE in catalog terms: which four parts make an electric vs a
 * steam engine. `composeProduct(world, recipe, engineParts(type))` turns that into a finished engine
 * product (the rig's starting engine seeds itself this way in `main.ts`); the bench produces the same
 * thing from parts the player dragged in.
 *
 * Two energy types ⇒ two profiles (the suggested, tunable numbers from the spec):
 *   - electric ≈ power 13 / torque 8 / weight 4 — high top speed, modest acceleration, light.
 *   - steam    ≈ power 8 / torque 19 / weight 8 — strong acceleration, lower top speed, heavy.
 * Engines now scale linearly (no diminishing returns) and weight is parked, so the contrast comes
 * straight from these profiles through `drive.ts`: at equal engine counts electric out-runs on top
 * speed and steam out-accelerates. The `EngineSpec` contract downstream is untouched.
 */

/** The recipe an engine of `type` is built from — the disjoint electric/steam slot vocabularies. */
export function engineRecipeForType(type: EnergyType) {
  return type === 'electric' ? ELECTRIC_ENGINE_RECIPE : STEAM_ENGINE_RECIPE;
}

/** The four catalog parts that compose an engine of `type`, in its recipe's slot order. */
export function engineParts(type: EnergyType): PartDef[] {
  return engineRecipeForType(type).slots.map((s) => {
    const def = PARTS_CATALOG.find((p) => p.slot === s.slot && p.type === type);
    if (!def) throw new Error(`no ${type} engine part for slot '${s.slot}'`);
    return def;
  });
}
