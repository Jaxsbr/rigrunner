...
import type { LootFind, LootRarity } from '../content/loot-table';
 ...
 * resolves to a catalog sub-part; granting is `spawnEnginePart` + `addToInventory`. When those tiers
  ...
  private collect(): void {
    ...
    for (const d of this.world.query(LootDrop)) {
      for (const find of this.world.get(d, LootDrop)!.finds) {
        ...
        if (def) addToInventory(this.world, spawnEnginePart(this.world, def));
      }
